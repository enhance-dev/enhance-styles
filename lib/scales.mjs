...
/**
 * @param {(string|number)} ratio - A named ratio or an explicit ratio
 * @param {string} scaleProperty - The scale property the ratio value will be used for; used in error reporting only
 * @returns {number} The numeric ratio value
 */
export function getRatioValue(ratio, scaleProperty) {
  // Config provided a raw number for the ratio
  if (typeof ratio === 'number') return ratio

  // Config provided a named value for the ratio
  if (Object.hasOwn(ratios, ratio)) return ratios[ratio]

  // Sad trombone
  throw new Error(`Value provided for ${scaleProperty} must be a number or a named ratio: ${Object.keys(ratios).join(', ')}`)
}
 ...
 * @param {number|string} config.scaleMin - The ratio, either as a number or a named ratio, to use at the minimum viewport width
 * @param {number|string} config.scaleMax - The ratio, either as a number or a named ratio, to use at the maximum viewport width
